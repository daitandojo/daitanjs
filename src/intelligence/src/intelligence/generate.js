import { query } from '@daitan/apiqueries';
import { isValidJSON } from '@daitan/validation';
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (error.response) {
        ...
        switch (status) {
          case 400:
            console.error(`Error 400 (Bad Request) during API call (Attempt ${i + 1}):`, data.error.message);
            return { error: 'Bad request - Invalid data provided.' };
          case 401:
            console.error(`Error 401 (Unauthorized) during API call (Attempt ${i + 1}):`, data.error.message);
            return { error: 'Unauthorized - Invalid API key or insufficient permissions.' };
          case 403:
            console.error(`Error 403 (Forbidden) during API call (Attempt ${i + 1}):`, data.error.message);
            return { error: 'Forbidden - You do not have access to this resource.' };
          case 404:
            console.error(`Error 404 (Not Found) during API call (Attempt ${i + 1}):`, data.error.message);
            return { error: 'Not Found - The requested resource could not be found.' };
          case 429:
            console.warn(`Error 429 (Too Many Requests) during API call (Attempt ${i + 1}):`, data.error.message);
            console.warn('Retrying after hitting rate limit...');
            break; // Retry after rate limiting
          case 500:
            console.error(`Error 500 (Internal Server Error) during API call (Attempt ${i + 1}):`, data.error.message);
            break; // Retry if server error
          default:
            console.error(`Error ${status} during API call (Attempt ${i + 1}):`, data.error.message);
            return { error: `Unexpected error occurred: ${data.error.message}` };
        }
      } else if (error.request) {
        // Request was made, but no response was received
        console.error(`No response received from OpenAI API (Attempt ${i + 1}):`, error.message);
      } else {
        // Something happened in setting up the request
        console.error(`Error setting up API call (Attempt ${i + 1}):`, error.message);
...
export async function fuzzyCompare(arrayOfStrings, targetString, specificInstruction) {
 ...
 */
const generateStory = async ({ genre, setting, mainCharacter, wordCount = 500 }) => {
  const instruction = `
...
const answerQuestion = async ({ instruction }) => {
  if (!instruction || typeof instruction !== 'string' || instruction.trim() === '') {
    throw new Error('Invalid instruction provided. Please provide a non-empty string.');
  }
...
const summarizeText = async ({
  ...
  const instruction = "You are a skilled summarizer. Provide a concise summary of the given text.";
...
const analyzeSentiment = async ({ text }) => {
  const instruction = "You are a sentiment analysis expert. Analyze the sentiment of the given text and provide a detailed breakdown.";
